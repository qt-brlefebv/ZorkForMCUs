#!/bin/bash
# build-flash-rt1050.sh
# Cross-compilation build and flash script for NXP RT1050-EVK with FreeRTOS
#
# Prerequisites:
#   - ARM GCC toolchain installed
#   - NXP MCUXpresso SDK for RT1050-EVK installed
#   - Qt for MCUs 2.12.0+ installed
#   - RT1050-EVK connected via debug probe
#
# Usage:
#   mkdir -p build-rt1050 && cd build-rt1050
...
BOARD_SDK_DIR="$(detect_rt1050_sdk)"
...
[[ -z "${BOARD_SDK_DIR}" ]] && die "NXP RT1050 SDK not found" \
...
# Display profile: RT1050 (480x272 landscape)
DISPLAY_PROFILE="${DISPLAY_PROFILE:-RT1050}"
...
echo "=== RT1050 FreeRTOS Build Configuration ==="
...
echo "=== Configuring CMake for RT1050 ==="
...
CMAKE_ARGS=(
    ...
    -DQUL_PLATFORM=mimxrt1050-evk-freertos
...
if [[ $? -ne 0 ]]; then
    ...
    echo ""
    echo "Common issues:"
    echo "  1. Platform not found: Verify mimxrt1050-evk-freertos is supported"
...
echo "=== Building ZorkUI for RT1050 ==="
ninja
...
if [[ $? -ne 0 ]]; then
    ...
    echo "Common issues:"
    echo "  1. Missing source files (fizmo_locale_stubs.c, story_data.S)"
...
if [[ -f "${ELF_FILE}" ]]; then
    ...
    # RT1050 has 64MB HyperFlash - plenty of space
    if [[ -f "${HEX_FILE}" ]]; then
